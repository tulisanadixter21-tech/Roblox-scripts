--[[ 
    MASTER HUB V18.2 | RIVALS DEFINITIVE EDITION
    - Feature: Custom Smooth Draggable UI
    - All features: Aimbot, Silent, ESP, Hitbox, Speed, Auto-Reload
]]

local S = {
    Aimbot = false, Silent = false, ESP = false, TeamCheck = true, 
    Fly = false, Hitbox = false, NoRecoil = false, SpeedBoost = false, 
    AutoReload = false, Smooth = 0.9, FOV = 180, WalkSpeed = 45, 
    JumpPower = 80, Running = true
}
...
main.BackgroundColor3 = Color3.fromRGB(12, 12, 15); main.BorderSizePixel = 0; Instance.new("UICorner", main)
...
RS.RenderStepped:Connect(function()
    ...
    if Aiming and Target then
        if S.Silent then Cam.CFrame = CFrame.new(Cam.CFrame.Position, Target.Position)
        elseif S.Aimbot then Cam.CFrame = Cam.CFrame:Lerp(CFrame.new(Cam.CFrame.Position, Target.Position), S.Smooth) end
    end
    ...
    if Char and Char:FindFirstChild("HumanoidRootPart") then
        ...
        elseif S.SpeedBoost and Hum then Hum.WalkSpeed = S.WalkSpeed; Hum.JumpPower = S.JumpPower
        elseif Hum then Hum.WalkSpeed = 16; Hum.JumpPower = 50 end
    end

    if S.Hitbox then
        for _, v in pairs(P:GetPlayers()) do
            if v ~= LP and v.Character and v.Character:FindFirstChild("Head") then
                v.Character.Head.Size = Vector3.new(3.5, 3.5, 3.5); v.Character.Head.CanCollide = false
            end
        end
    end
end)
        ...
        if p.Character then
            ...
            task.spawn(function()
                while p.Character and h.Parent and S.Running do
                    h.Enabled = S.ESP and (not S.TeamCheck or p.Team ~= LP.Team); task.wait(0.2)
                end
            end)
        end
    end
