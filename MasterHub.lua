--[[ 
    MASTER HUB UNIVERSAL v2.1
    - RIVALS: Full Snappy/Silent Aim, Team-Check ESP, Hitbox Expander, FOV Scale, No Recoil, Auto Reload
    - BLOX FRUITS: Auto-Quest (NEW), Auto-Farm, Bring Mobs, Auto-Click, Fast Attack
...
-- 2. MASTER SETTINGS (RIVALS + BLOX FRUITS)
local S = {
    Running = true, Fly = false, SpeedBoost = false, InfJump = true,
    -- RIVALS SETTINGS (DO NOT REMOVE)
    ...
    AutoFarm = false, AutoQuest = false, AutoClick = false, BringMobs = false, FastAttack = false
}

if CG:FindFirstChild("MasterHub") then CG.MasterHub:Destroy() end

-- 3. RIVALS VISUALS (FOV & CROSSHAIR)
...
-- 6. DYNAMIC UI BUTTONS (RIVALS VS BLOX FRUITS)
...
elseif GameMode == "BloxFruits" then
    MB("🚜 AUTO QUEST", "AutoQuest", Color3.fromRGB(0, 255, 100))
...
-- 7. THE ENGINE (RIVALS + BLOX FRUITS)
local Aiming = false
...
RS.RenderStepped:Connect(function()
    ...
    if GameMode == "Rivals" then
        local T = GetT()
        Circle.Position = Vector2.new(Cam.ViewportSize.X/2, Cam.ViewportSize.Y/2); Circle.Radius = GetScaledFOV(); Circle.Visible = (S.Aimbot or S.Silent); Dot.Position = Circle.Position; Dot.Visible = Circle.Visible; Circle.Color = T and Color3.fromRGB(255, 50, 50) or Color3.fromRGB(0, 255, 150)
        ...
        if S.NoRecoil and Aiming then LP.CameraMaxZoomDistance = LP.CameraMaxZoomDistance end
    ...
    if GameMode == "BloxFruits" then
        ...
        if S.BringMobs then for _, v in pairs(workspace.Enemies:GetChildren()) do if v:FindFirstChild("HumanoidRootPart") and (v.HumanoidRootPart.Position - LP.Character.HumanoidRootPart.Position).Magnitude < 300 then v.HumanoidRootPart.CFrame = LP.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,-5); v.HumanoidRootPart.CanCollide = false end end end
    end
