--[[
╔══════════════════════════════════════════════════════════════════════════════╗
║   MASTER HUB · RIVALS · V5.1 "MOBILE ULTIMATE FIX"                          ║
║                                                                              ║
║   COMPLETE FIXED VERSION - READY TO PASTE                                    ║
║   ✓ Working floating toggle button                                           ║
║   ✓ Edge swipe controls                                                      ║
...
-- ============================================
-- FIXED FLOATING BUTTON (MULTIPLE TOUCH METHODS)
-- ============================================
...
toggleButton.Size = UDim2.new(0, IS_MOBILE and 70 or 50, 0, IS_MOBILE and 70 or 50)
toggleButton.Position = UDim2.new(1, -(IS_MOBILE and 90 or 70), 1, -(IS_MOBILE and 120 or 90))
...
toggleIcon.TextSize = IS_MOBILE and 36 or 28
...
task.spawn(function()
    while sg.Parent do
        tween(pulse, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Size = UDim2.new(1.3, 0, 1.3, 0),
            Position = UDim2.new(-0.15, 0, -0.15, 0),
            BackgroundTransparency = 1
        })
        task.wait(1)
        tween(pulse, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local function toggleUI()
    ...
    -- Visual feedback
    tween(toggleButton, TweenInfo.new(0.3), {
        BackgroundColor3 = S.UIVisible and C.purple or Color3.fromRGB(100, 100, 100)
    })
    
    -- Haptic feedback
    if IS_MOBILE then
        pcall(function()
            UserInputService:VibrationMotor("large", 0.1, 0.1)
        end)
    end
end

-- METHOD 1: TextButton overlay (most reliable)
...
touchButton.Text = ""
touchButton.Parent = toggleButton
touchButton.ZIndex = 1001
touchButton.AutoButtonColor = false

-- Visual feedback
touchButton.MouseButton1Down:Connect(function()
    tween(toggleButton, TweenInfo.new(0.1), {Size = UDim2.new(0, 60, 0, 60)})
    toggleIcon.TextSize = 30
end)

touchButton.MouseButton1Up:Connect(function()
    tween(toggleButton, TweenInfo.new(0.2, Enum.EasingStyle.Bounce), {Size = UDim2.new(0, 70, 0, 70)})
    toggleIcon.TextSize = 36
end)

touchButton.MouseButton1Click:Connect(toggleUI)

-- METHOD 2: Direct TouchTap
toggleButton.TouchTap:Connect(toggleUI)

-- METHOD 3: Three-finger tap
local tapCount = 0
local tapTimer
...
UserInputService.TouchTap:Connect(function(taps, gameProcessed)
    ...
    tapTimer = task.delay(0.5, function()
        if tapCount >= 3 then
            toggleUI()
        end
        tapCount = 0
...
UserInputService.TouchTap:Connect(function(tap)
    swipeStart = tap.Position
end)

UserInputService.TouchMoved:Connect(function(tap)
    if swipeStart then
        local delta = tap.Position - swipeStart
        
        -- Swipe from left edge to open
        if swipeStart.X < 50 and delta.X > swipeThreshold then
            S.UIVisible = true
        ...
        if swipeStart.X > Cam.ViewportSize.X - 50 and delta.X < -swipeThreshold then
            S.UIVisible = false
...
do
    ...
    local versionVal = infoRow("Version", "v5.1 Mobile")
...
-- FINAL MESSAGE
-- ============================================
print("✅ Master Hub V5.1 Mobile Ultimate loaded!")
print("📱 Toggle methods:")
print("   • Tap purple button")
print("   • Swipe from left edge to open")
print("   • Swipe from right edge to close")
print("   • Three-finger tap anywhere")